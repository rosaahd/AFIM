...
const PASSWORD = process.env.PASSWORD // 'manager'
const DB_NAME = process.env.DB_NAME //'AFIM'
const DB_HOST = process.env.DB_HOST // 'cluster0.rd6s0qb.mongodb.net'
...
class MongoLib {
    ...
    async delete(collection, id) {
        try {
            let db = await this.connect();
            let result = await db.collection(collection).deleteOne({ id: id });
